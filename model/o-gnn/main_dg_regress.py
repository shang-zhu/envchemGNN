...
def main():
    ...
    if args.save_test:
        with open('result/'+task_name+'/test_idx_'+str(kfold_idx)+'.txt', 'a+') as fp:
            for idx in test_idx:
                fp.write("%s\n" % idx)
    ...
    if len(val_idx)==0:
        ...
        for train_index, val_index in kf.split(train_idx):
            ...
            for epoch in range(_start_epoch + 1, args.epochs + 1):
                ...
                if valid_mae < best_valid_mae:
                    ...
                    if args.save_test: 
                        torch.save(model.state_dict(), 'result/'+task_name+'/model_k'+str(kfold_idx)+'_n'\
                        ...
                        if name_appendix==0:
                            ...
                            test_df=pd.read_csv('result/'+task_name+'/preds_'+str(kfold_idx)+'.csv')
                            test_df['y_pred'+str(name_appendix)]=list(y_pred.cpu().detach().numpy())
                        test_df.to_csv('result/'+task_name+'/preds_'+str(kfold_idx)+'.csv', index=False)
        ...
        for epoch in range(_start_epoch + 1, args.epochs + 1):
            ...
            if valid_mae < best_valid_mae:
                ...
                if args.save_test: 
                    torch.save(model.state_dict(), 'result/'+task_name+'/model.pt')
                    print("Predicting on test data...")
                    y_true, y_pred = test(model, device, test_loader, args)
                    test_df=pd.DataFrame.from_dict({'y_true':list(y_true.cpu().detach().numpy()),\
